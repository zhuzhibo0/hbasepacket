...
public
  ...
  PcapsResponse processKey(PcapsResponse pcapsResponse, String key,
    ...
    try {
      ...
      if (keysMap == null) {
        ...
        for (Cell cell : cells) {
          pcapsResponse.addPcaps(PcapUtils.cellToPcap(cell));
        }
        return pcapsResponse;
      }
      ...
      System.out.println("scannedCells size :" + scannedCells.size());
      addToResponse(pcapsResponse, scannedCells, maxResultSize);

    } catch (IOException e) {
    	System.out.println("Exception occurred while fetching Pcaps for the keys :"
          + key+e.toString());
  ...
  private void addToResponse(PcapsResponse pcapsResponse,
    ...
    for (Cell sortedCell : scannedCells) {
      pcapsResponse.addPcaps(PcapUtils.cellToPcap(sortedCell));
    }
