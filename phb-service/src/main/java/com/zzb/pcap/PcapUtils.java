...
public class PcapUtils {
  ...
  public static void addInt(List<Byte> list,int number) {
  ...
  public static void createPktHeader(List<Byte> list,long ts,int len) {
		    ...
		    int sec = (int) (ts/1000);
		    int usec = (int) (ts%1000);
		    addInt(list,sec);
		    addInt(list,usec);
		    addInt(list,len);
		    addInt(list,len);
	}
  public static byte[] cellToPcap(Cell scell)
	  {
		  long ts = scell.getTimestamp();
		  byte[] rawpkt = CellUtil.cloneValue(scell);
		  //
		  List<Byte> list = new ArrayList<Byte>();
		  
		  createPktHeader(list,ts,rawpkt.length);
		 
	    for (int i = 0; i < rawpkt.length; i++) { 
	        list.add(rawpkt[i]);
	      }
